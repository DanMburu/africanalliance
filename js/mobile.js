...
sql = "DROP TABLE IF EXISTS aa_program";
   ...
   sql = "CREATE TABLE IF NOT EXISTS aa_program ( "+
    ...
    tx.executeSql(sql);
	  ...
	  tx.executeSql(sql);
	  sql = "CREATE TABLE IF NOT EXISTS aa_program_days ( "+
			  "id INTEGER PRIMARY KEY AUTOINCREMENT, " +
			  "date VARCHAR(200), " +
...
function GetDates(){
	...
	db.transaction(function(transaction) {
		transaction.executeSql("select * from aa_program_days", [],
				function(tx, result) { // On Success
					var len = result.rows.length;

					for (var i=0; i<len; i++) {
						var row = result.rows.item(i);
                        $('.date-'+row.id).text(row.date);
						$('.day-'+row.id).text(row.DayOfTheWeek);
					}// End for
				},
				function(error){
					console.log(error);
